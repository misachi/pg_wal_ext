...
CREATE FUNCTION pg_xlog_records(IN xlog_file_path text, OUT page_num int4, OUT txn_id xid, OUT xlog_type text, OUT commit_ts timestamptz, OUT generated_sql text)
...
-- Query example
-- select * from pg_xlog_records(path/to/wal/file); => where `path/to/wal/file` is the full path to the WAL file
